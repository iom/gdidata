#' Irregular Migrant Arrivals to Honduras
#'
#' Irregular migrant arrivals to Honduras as reported by the government.
#'
#' \describe{
#'   \item{t}{month-year of count}
#'   \item{n}{count of individuals}
#' }
#'
#' @details Scraped from the Honduras National Institute for Migration's *Flujo
#'   Migratorio Irregular* dashboard
#'   (https://app.powerbi.com/view?r=eyJrIjoiODNhMGY4MGUtM2NkMy00YTIzLTlmZTAtNmQxZmJiZDhiZjc1IiwidCI6ImUxMWQ5NjIwLTRkM2UtNDEwYi05MTUyLWZkMWNmNmNmNzI5YSJ9).
#'   Period covered is from 2014 Jan to 2025 Apr
#'
#'   Last updated 11 Jun 2025.
#'
#' @source Honduras National Institute for Migration; DTM Americas.
"irreg_honduras"


#' Irregular Migrant Arrivals to Honduras: Nationalities
#'
#' Irregular migrant arrivals to Honduras as reported by the government, by
#' nationality.
#'
#' \describe{
#'   \item{t}{year of count}
#'   \item{nat}{nationality}
#'   \item{n}{count of individuals}
#' }
#'
#' @details Scraped from the Honduras National Institute for Migration's *Flujo
#'   Migratorio Irregular* dashboard
#'   (https://app.powerbi.com/view?r=eyJrIjoiODNhMGY4MGUtM2NkMy00YTIzLTlmZTAtNmQxZmJiZDhiZjc1IiwidCI6ImUxMWQ5NjIwLTRkM2UtNDEwYi05MTUyLWZkMWNmNmNmNzI5YSJ9).
#'   Period covered is from 2018 to 2024 and Jan-Apr 2025.
#'
#'   Last updated 13 May 2025.
#'
#' @source Honduras National Institute for Migration.
"irreg_honduras_nat"


#' Irregular Migrant Arrivals to Honduras: Nationalities (Monthly)
#'
#' Irregular migrant arrivals to Honduras as reported by the government, by
#' nationality. Monthly version of `irreg_honduras_nat`.
#'
#' \describe{
#'   \item{t}{year-month of count}
#'   \item{nat}{nationality}
#'   \item{n}{count of individuals}
#' }
#'
#' @details Scraped from the Honduras National Institute for Migration's *Flujo
#'   Migratorio Irregular* dashboard
#'   (https://app.powerbi.com/view?r=eyJrIjoiODNhMGY4MGUtM2NkMy00YTIzLTlmZTAtNmQxZmJiZDhiZjc1IiwidCI6ImUxMWQ5NjIwLTRkM2UtNDEwYi05MTUyLWZkMWNmNmNmNzI5YSJ9).
#'   Period covered is from Jan 2022 to Apr 2025.
#'
#'   Last updated 11 Jun 2025.
#'
#' @source Honduras National Institute for Migration.
"irreg_honduras_nat_m"


#' Irregular Migrant Arrivals to Honduras: Demographics
#'
#' Irregular migrant arrivals to Honduras as reported by the government, by sex
#' and age.
#'
#' \describe{
#'   \item{t}{year of count}
#'   \item{demog}{men, women, boys, girls}
#'   \item{n}{count of individuals}
#' }
#'
#' @details Scraped from the Honduras National Institute for Migration's *Flujo
#'   Migratorio Irregular* dashboard
#'   (https://app.powerbi.com/view?r=eyJrIjoiODNhMGY4MGUtM2NkMy00YTIzLTlmZTAtNmQxZmJiZDhiZjc1IiwidCI6ImUxMWQ5NjIwLTRkM2UtNDEwYi05MTUyLWZkMWNmNmNmNzI5YSJ9).
#'   Period covered is from 2018 to 2024 and Jan-Apr 2025.
#'
#'   Last updated 13 May 2025.
#'
#' @source Honduras National Institute for Migration.
"irreg_honduras_dmg"


#' Irregular Migrant Arrivals to Honduras: Demographics (Monthly)
#'
#' Irregular migrant arrivals to Honduras as reported by the government, by sex
#' and age. Monthly version of `irreg_honduras_dmg_m`.
#'
#' \describe{
#'   \item{t}{year-month of count}
#'   \item{demog}{men, women, boys, girls}
#'   \item{n}{count of individuals}
#' }
#'
#' @details Scraped from the Honduras National Institute for Migration's *Flujo
#'   Migratorio Irregular* dashboard
#'   (https://app.powerbi.com/view?r=eyJrIjoiODNhMGY4MGUtM2NkMy00YTIzLTlmZTAtNmQxZmJiZDhiZjc1IiwidCI6ImUxMWQ5NjIwLTRkM2UtNDEwYi05MTUyLWZkMWNmNmNmNzI5YSJ9).
#'   Period covered is from Jan 2022 to Apr 2025.
#'
#'   Last updated 11 Jun 2025.
#'
#' @source Honduras National Institute for Migration.
"irreg_honduras_dmg_m"

